...
def _load_confirmations(filename: str) -> list[dict[str, Any]]:
    """Load the held-out test set: each confirmation is ``{meal_text, items,
    totals}`` where the totals are the agent's USDA-grounded estimate of a
    precise spec of that meal — the real ground truth, not a hand-typed number
    (generated by tools/gen_demo_data.py)."""
    return _load(filename)["confirmations"]
...
RUNNER = Persona(
    ...
    # Held-out test set (generated from real agent output, validated live): pre-run
    # she carb-loads, post-run she eats heavy protein — both run well above the
    # agent's default servings, while the plain lunch guard must stay put. Each
    # confirmation's true calories are the agent's estimate of a precise spec.
...
BODYBUILDER = Persona(
    ...
    # Held-out test set (real agent output): post-lift protein meals the agent
    # under-portions (the pattern to learn), plus non-workout guards the learned
    # rule must NOT change. True calories = the agent's estimate of a precise spec.
